function [CylinderJ EndPlate1 EndPlate2] = Cylinder(X1,X2,r,n,cyl_color,closed,lines,orient)
%
...
if orient==0
    ...
    if angle_X1X2~=0 % Rotation is not needed if required direction is along X
        rotate(CylinderJ,axis_rot,angle_X1X2,[0 0 0])
        if closed==1
...
elseif orient==1
    ...
    if closed==1
            hold on;
            EndPlate1=fill3(xx2(:,1),xx1(:,1),xx3(:,1),'r');
            EndPlate2=fill3(xx2(:,2),xx1(:,2),xx3(:,2),'r');
    end
    CylinderJ=mesh(xx2,xx1,xx3);
 %{
    unit_Vy=[0 1 0];
    angle_X2X3 = acos( dot( unit_Vy,(X2-X1) )/( norm(unit_Vy)*norm(X2-X1)) )*180/pi;
    axis_rot = cross([0 1 0],(X2-X1) );
    if angle_X2X3~=0 % Rotation is not needed if required direction is along X
        rotate(CylinderJ,axis_rot,angle_X2X3,[0 0 0])
        if closed==1
            rotate(EndPlate1,axis_rot,angle_X2X3,[0 0 0])
            rotate(EndPlate2,axis_rot,angle_X2X3,[0 0 0])
        end
    end
   %}
elseif orient==2
    ...
    unit_Vz=[1 0 0];
    angle_X3X1 = acos( dot( unit_Vz,(X2-X1) )/( norm(unit_Vz)*norm(X2-X1)) )*180/pi;
    %axis_rot = [0 0 1];
    axis_rot = cross([0 1 0],(X2-X1) );
    if angle_X3X1~=0 % Rotation is not needed if required direction is along X
        rotate(CylinderJ,axis_rot,-angle_X3X1,[0 0 0])
        if closed==1
                rotate(EndPlate1,axis_rot,angle_X3X1,[0 0 0])
                rotate(EndPlate2,axis_rot,angle_X3X1,[0 0 0])
        end
    end
...
%Cylinder=mesh(xx1,xx2,xx3);  --- apagado por mim
...
set(CylinderJ,'XData',get(CylinderJ,'XData')+X1(1))
set(CylinderJ,'YData',get(CylinderJ,'YData')+X1(2))
set(CylinderJ,'ZData',get(CylinderJ,'ZData')+X1(3))

% Setting the color to the cylinder and the end plates
set(CylinderJ,'FaceColor',cyl_color)
if closed==1
